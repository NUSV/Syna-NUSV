...
- **Honest boundary**: device-owner-level monitoring (pre-installed spyware / MDM) and kernel-level rootkits (forged /proc) cannot be detected by an app; the fail-closed gate, data-level key gate, native anti-hook and self-destruct are the compensating controls — see [shield-design.md §16](../security/shield-design.md#16-honest-boundary-again-plainly). Protection stays effective under full disclosure (GPL-3.0).
